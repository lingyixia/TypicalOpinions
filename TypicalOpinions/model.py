...
class SummarizationModel(object):
    ...
    def _reduce_states(self, fw_st, bw_st, emb_categoryWord, emb_tagWords, tagWordsLen):
        ...
        with tf.variable_scope('reduce_final_st'):
            ...
            bias_reduce_h = tf.get_variable('bias_reduce_h', [hidden_dim], dtype=tf.float32,
                                            initializer=self.trunc_norm_init)
            # Apply linear layer
            old_c = tf.concat(axis=1, values=[fw_st.c, bw_st.c, emb_tagWordsEnv])  # Concatenation of fw and bw cell
            old_h = tf.concat(axis=1, values=[fw_st.h, bw_st.h, emb_tagWordsEnv])  # Concatenation of fw and bw state
    ...
    def _add_seq2seq(self):
        ...
        with tf.variable_scope('seq2seq'):
            ...
            with tf.variable_scope('embedding'):
                ...
                # list length max_dec_steps containing shape (batch_size, emb_size)(35个(16,128)的list)
            ...
            # Add the encoder.(enc_outputs(16,35,256),fw_st(35,256),bw_st(35,256))
            enc_outputs, fw_st, bw_st = self._add_encoder(emb_enc_inputs, self._enc_lens)
            self._enc_states = enc_outputs  # (16,?,512)
            # Our encoder is bidirectional and our decoder is unidirectional so we need to reduce the final encoder hidden state to the right size to be the initial decoder hidden state 其实就是转换了一下维度,否则没法作为decode的输入，所有双向的都可以这么做
            ...
            with tf.variable_scope('output_projection'):
                ...
                for i, output in enumerate(decoder_outputs):
                    ...
                    vocab_scores.append(tf.nn.xw_plus_b(output, w, v))  # apply the linear layer#公式(4)
                ...
                with tf.variable_scope('loss'):
                    if FLAGS.pointer_gen:
                        ...
                        for dec_step, dist in enumerate(final_dists):
                            targets = self._target_batch[:, dec_step]  # (batch_size)
                            indices = tf.stack((batch_nums, targets), axis=1)  # shape (batch_size, 2)
                            gold_probs = tf.gather_nd(dist, indices)  # shape (batch_size)
    ...
    def decode_onestep(self, sess, batch, latest_tokens, enc_states, dec_init_states, prev_coverage):
        ...
        Args:
          ...
          enc_states: The encoder states.(en_outbatch)
