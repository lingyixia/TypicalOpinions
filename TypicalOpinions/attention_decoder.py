...
def attention_decoder(decoder_inputs, initial_state, emb_categoryWord, categoryWord, en_batch, encoder_states,
    ...
    Args:
      ...
      encoder_states: 3D Tensor [batch_size x attn_length x attn_size].(是en_output)
    ...
    with variable_scope.variable_scope("attention_decoder") as scope:
        ...
        # Reshape encoder_states (need to insert a dim) encoder_states是encoder的输出(16,?,256)(256由双向contact起来得到)
        ...
        #W_t = variable_scope.get_variable("W_t", [1, 1, 128, attention_vec_size])
        #emb_categoryWord = tf.expand_dims(tf.expand_dims(emb_categoryWord, axis=1), axis=1)
        #category_features = nn_ops.conv2d(emb_categoryWord, W_t, [1, 1, 1, 1], "SAME")
        # W_category = variable_scope.get_variable("W_category", [categoryWord.shape[1], attention_vec_size])
        ...
        for i, inp in enumerate(decoder_inputs):
            ...
            if i == 0 and initial_state_attention:  # always true in decode mode
                ...
                context_vector, attn_dist, coverage = attention(state, coverage)  # 公式(3) 公式(2)和公式(10)
